...
type UnionConverter (discriminator : string, ?catchAllCase) =
    inherit JsonConverter()

    let isList (y: Type) = y.IsGenericType && typedefof<List<_>> = y.GetGenericTypeDefinition ()
    ...
    override __.CanConvert (t: Type) = Union.isUnion t && not (isList t)
    ...
    override __.WriteJson(writer: JsonWriter, value: obj, jsonSerializer: JsonSerializer) =
        ...
        match union.cases, fieldValues with
        | [|_|], [| single |] -> jsonSerializer.Serialize(writer, single)
        | _ -> 
    ...
    override __.ReadJson(reader: JsonReader, t: Type, _: obj, jsonSerializer: JsonSerializer) =        
        ...
        match union.cases, union.cases.[0].GetFields() with
        | [|singleCase|], [|singleField|] ->
            let scdu = jsonSerializer.Deserialize(reader, singleField.PropertyType)
            FSharpValue.MakeUnion(singleCase, [| scdu |], true)
        | _ ->
